	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div class="card-header text-white border-0 py-3 px-4"
			...
			<h5 class="mb-0 fw-bold">
				...
				<ors:message key="marksheet.meritlist.title" />
			</h5>
		</div>
			...
			<div class="table-responsive">
				<table class="table table-hover align-middle mb-0">
					<thead class="table-light">
						<tr>
							<th width="60" class="text-center"><ors:message
									key="marksheet.rank" /></th>
					...
					<tbody>
						...
						<tr>
							<td class="text-center">
								<% if(rank == 1) { %> <span
								class="badge bg-warning text-dark fs-6"><i
									class="bi bi-trophy-fill"></i> <%=rank++%></span> <% } else if(rank == 2) { %>
								<span class="badge bg-secondary fs-6"><%=rank++%></span> <% } else if(rank == 3) { %>
								<span class="badge"
								style="background: #cd7f32; font-size: 1rem;"><%=rank++%></span>
								<% } else { %> <span class="text-muted"><%=rank++%></span> <% } %>
							</td>
							<td class="fw-semibold"><%=bean.getRollNo()%></td>
							<td><%=bean.getName()%></td>
							<td class="text-center"><span
								class="badge bg-primary rounded-pill px-3"><%=bean.getPhysics()%></span></td>
							<td class="text-center"><span
								class="badge bg-success rounded-pill px-3"><%=bean.getChemistry()%></span></td>
							<td class="text-center"><span
								class="badge bg-warning text-dark rounded-pill px-3"><%=bean.getMaths()%></span></td>
						</tr>
						<% } %>
					</tbody>
				</table>
