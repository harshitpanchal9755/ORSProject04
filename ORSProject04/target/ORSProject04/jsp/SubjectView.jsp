...
<jsp:useBean id="bean" class="com.sunilos.p4.bean.SubjectBean"
	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div class="card-header text-white border-0 py-3 px-4"
			...
			<h5 class="mb-0 fw-bold">
				...
				<%=bean.getId() > 0 ? "Edit Subject" : "Add Subject"%>
			</h5>
		</div>
		...
		<div class="card-body px-4 py-4">
			...
			<form action="<%=ORSView.SUBJECT_CTL%>" method="POST">
				...
				<div class="mb-4">
					<label class="form-label fw-semibold">Course <span
						class="text-danger">*</span></label> <select name="courseId"
						class="form-select">
            ...
            %>
						<option value="<%=course.getId()%>"
							<%=(bean.getCourseId() == course.getId()) ? "selected" : ""%>>
							<%=course.getName()%>
						</option>
						<%
                }
            ...
            %>
					...
					<div class="text-danger small mt-1"><%=ServletUtility.getErrorMessage("courseId", request)%></div>
				</div>
				...
				<div class="d-flex gap-2 pt-2 border-top">
					...
					<% } %>
					<a href="<%=ORSView.SUBJECT_LIST_CTL%>"
